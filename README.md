...
- **Install updates**: select one or more pending `update.*` entities, calls `portainer_maintenance.perform_update` for each.
  - *(1.3.5)* A large batch used to be processed inside one blocking HTTP request — every selected update in sequence, one connection held open for however long the whole thing took. A big batch (or a single slow item in it) could keep that connection open for minutes, at the mercy of whatever reverse proxy or browser timeout sits in front of this app; when one fired, the *entire* batch got reported as failed even though the server-side loop was still correctly working through it — this, not actual concurrent-update contention, is what broke a large non-stack batch reported in the field (nothing here has ever sent Portainer more than one recreate at a time). Install now submits the whole batch and returns almost immediately with one job per update, queued one-per-Portainer-endpoint — two endpoints' batches now run concurrently, though a single endpoint's updates still go out strictly one at a time, same as before, since that's what's actually safe for its Docker daemon/Portainer agent. The webapp polls for status and clears each row's "Installing…" state the moment its own job resolves, not when the slowest job in the whole batch finally does, and each job's own timeout is scoped from the moment it actually starts running rather than from whenever the batch was submitted.
